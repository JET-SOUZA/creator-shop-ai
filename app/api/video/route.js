...
export async function POST(request) {
  try {
    ...
    const duration = Math.max(1, Math.min(5, Number(body.duration) || 5))
    const aspectRatio = ['9:16','1:1','16:9'].includes(body.aspectRatio) ? body.aspectRatio : '9:16'
    const promptText = String(body.prompt || '').trim()
    if (!promptText) return Response.json({ ok:false, error:'Descreva a ideia antes de gerar o clipe.' }, { status:400 })
    ...
    return new Response(streamBytes(video.uint8Array), {
      headers: {
        ...
        'Content-Disposition':'inline; filename="legacy-creator-clip.mp4"',
        'Cache-Control':'no-store',
        'X-Legacy-Model':'spacexai/grok-imagine-video'
      }
    })
  } catch (error) {
    console.error('legacy-video-error', error)
    const detail = NoVideoGeneratedError?.isInstance?.(error) ? 'O provedor não conseguiu gerar o vídeo.' : (error?.message || 'VIDEO_GENERATION_ERROR')
    return Response.json({ ok:false, error:'Não foi possível gerar o clipe agora.', detail, hint:'A geração de vídeo usa créditos do AI Gateway e pode exigir faturamento habilitado.' }, { status:500 })
  }
}
